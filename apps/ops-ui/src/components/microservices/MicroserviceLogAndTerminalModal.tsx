      ...
      <div className="space-y-4">
        ...
        {activeTab === 'logs' && (
          <div className="space-y-3">
            ...
            {/* Log Stream Viewer */}
            <div className="bg-surface border border-border rounded-xl p-4 font-mono text-xs text-foreground/90 h-96 overflow-y-auto space-y-1.5 scrollbar-hide">
              {filteredLogs.length === 0 ? (
                <div className="text-center py-12 text-foreground/40">No logs found for current filter</div>
              ) : (
                filteredLogs.map((l) => (
                  <div key={l.id} className="flex items-start gap-3 hover:bg-surface-2 p-1 rounded">
                    <span className="text-foreground/30 text-[10px] shrink-0 font-mono">{new Date(l.timestamp).toLocaleTimeString()}</span>
                    <span
        ...
        {/* TERMINAL TAB */}
        {activeTab === 'terminal' && (
          <div className="bg-surface border border-border rounded-xl p-2 h-96 overflow-hidden">
