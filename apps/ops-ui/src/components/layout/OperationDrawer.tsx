        ...
        >
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-border/40 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-foreground/80">Operations</span>
              <span className="text-[10px] text-foreground/40 font-mono">{events.length} events</span>
            </div>
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="w-5 h-5 flex items-center justify-center rounded text-foreground/40 hover:text-foreground/80 hover:bg-surface-2/80 transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            {events.map((ev) => (
              <div key={ev.id} className="group">
                <button
                  ...
                  className="w-full flex items-start gap-2.5 px-3 py-2 hover:bg-surface-2 transition-colors text-left"
                >
                  {/* Type icon */}
                  <span className="text-[11px] font-mono text-foreground/30 shrink-0 w-5 text-center mt-0.5">
                    {TYPE_ICONS[ev.type] ?? '•'}
                  </span>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {/* Severity badge */}
                      <span className={cn('text-[10px] font-bold shrink-0', SEVERITY_STYLES[ev.severity] ?? 'text-foreground/50')}>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      {ev.serverName && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-surface-2 text-foreground/40">
                          {ev.serverName}
                        </span>
                      )}
                      {ev.appName && (
                    ...
                    {expanded === ev.id && (
                      <div className="mt-2 space-y-1.5">
                        {ev.description && ev.description !== ev.title && (
                          <p className="text-[11px] text-foreground/50 break-words">{ev.description}</p>
                        )}
                        {ev.metadata && (
                          <pre className="text-[10px] font-mono text-green-400/70 bg-surface-2 rounded p-2 overflow-x-auto whitespace-pre-wrap break-all max-h-40 overflow-y-auto">
      ...
      <button
        ...
        className={cn(
          ...
          open
            ...
            : 'bg-black/90 text-foreground/70 hover:text-foreground',
        )}
      ...
      >
        ...
        <span
          className={cn(
            ...
            hasNew ? 'bg-green-400 animate-pulse' : 'bg-border/60',
          )}
        />
        <span>Ops Log</span>
        {events.length > 0 && (
          <span className="px-1.5 py-0.5 rounded-full bg-surface-2/80 text-[10px] font-mono">
            {events.length}
          </span>
        )}
      </button>
