...
export default function LogsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Log Output Console */}
      <div className="bg-[#0b0f19] border border-border rounded-xl p-4 font-mono text-xs text-white/90 h-[520px] overflow-y-auto space-y-1 scrollbar-hide">
        {loading ? (
          <div className="p-8 text-center text-white/40">Loading log output...</div>
        ) : filteredLines.length === 0 ? (
          <div className="p-8 text-center text-white/40">No logs found for current source/filter</div>
        ) : (
          filteredLines.map((l, i) => (
            <div key={i} className="hover:bg-white/5 px-2 py-0.5 rounded whitespace-pre-wrap break-all">
              {l}
            </div>
          ))
        )}
